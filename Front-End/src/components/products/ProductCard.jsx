...
function ProductCard({ product }) {
  ...
  return (
    <article className="group relative h-74.5 w-47 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">
      {/* Wishlist button */}
      <button
        onClick={() => toggleWishlist(product)}
        className="absolute right-2 top-2 z-10 rounded-full bg-white/80 p-1.5 shadow backdrop-blur transition hover:bg-white"
      ...
      <Link to={`/product/${product.id}`}>
        <img
          ...
          className="h-50 w-full rounded object-cover transition duration-300 group-hover:scale-[1.02]"
        />
      </Link>
      ...
      <div className="space-y-2 p-3">
        ...
        <div className="flex items-center justify-between pt-1 ">
          ...
          <button
            ...
            className="rounded-lg bg-indigo-600 px-1 py-1 text-[10px] font-semibold text-white transition hover:bg-indigo-700 active:scale-95"
          >
            Add to cart
          </button>
        </div>
